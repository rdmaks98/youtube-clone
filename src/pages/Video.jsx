...
const Video = () => {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        const videoResult = await axios.get(`/videos/find/${path}`)
        console.log(videoResult,2442)
        const channelResult = await axios.get(`/users/find/${videoResult?.data?.userId}`)
        setChannel(channelResult?.data);
        dispatch(fetchSuccess(videoResult?.data))
      } catch (err) {
